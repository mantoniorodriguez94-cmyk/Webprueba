...
export default function BusinessDetailPage() {
  ...
  // Heal: if business has owner_id but tier is missing, fetch profile in background
  ...
  return (
    <div className="min-h-screen pb-12">
      ...
      <SectionHeader
        ...
        subtitulo={business.category || "Negocio"}
        ancho="7xl"
        onVolver={() => router.back()}
        acciones={
          <>
            {/* Reportar vive en un menú y no entre las acciones principales:
                un botón rojo visible sugiere que reportar es algo esperado. */}
            {user && !isOwner && (
        ...
        <div className="surface rounded-3xl shadow-sm p-5 sm:p-6 mb-6">
          ...
          {!isOwner && (
            <div className="flex flex-col sm:flex-row gap-3 mt-6 pt-6 border-t border-black/8">
              ...
              {/* Enviar Mensaje: Tier 0 = disabled + tooltip; Tier 1+ = normal */}
              {!isOwner && (
                !ownerHasChat ? (
                  <button
                    type="button"
