  ...
  if (userLoading || loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center surface-elevated rounded-3xl p-12">
          <div className="animate-spin rounded-full h-16 w-16 border-b-4 border-blue-500 mx-auto"></div>
          <p className="mt-4 text-ink-2 font-medium">Cargando estadísticas...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-green-50 rounded-3xl border border-green-200 p-6 hover:border-green-300 transition-all shadow-sm">
            ...
            <h3 className="text-4xl font-extrabold text-ink mb-1">{whatsappClicks.toLocaleString()}</h3>
          ...
          <div className="bg-blue-50 rounded-3xl border border-blue-200 p-6 hover:border-blue-300 transition-all shadow-sm">
            ...
            <h3 className="text-4xl font-extrabold text-ink mb-1">{analytics.total_views.toLocaleString()}</h3>
            <p className="text-sm text-blue-700 font-semibold">Visitas al perfil</p>
            <p className="text-xs text-ink-2 mt-2">{analytics.unique_viewers.toLocaleString()} visitantes únicos</p>
          </div>
          ...
          <div className="bg-purple-50 rounded-3xl border border-purple-200 p-6 hover:border-purple-300 transition-all shadow-sm">
            ...
            <h3 className="text-4xl font-extrabold text-ink mb-1">{favoritesCount.toLocaleString()}</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 surface rounded-3xl p-8 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-xs text-ink-2 bg-black/5 px-3 py-1.5 rounded-full font-medium">Últimos 30 días</span>
            </div>
            ...
            {viewsByDay.length > 0 ? (
                  ...
                  return (
                    <div key={index} className="group">
                      <div className="flex items-center gap-3">
                        <span className="text-sm text-ink-2 w-20 text-right font-medium">
                          {new Date(day.view_date).toLocaleDateString('es-ES', { month: 'short', day: 'numeric' })}
                        </span>
                        <div className="flex-1 relative">
                          <div className="bg-black/5 rounded-full h-10 overflow-hidden border border-black/5">
                            <div 
                              className="bg-blue-500 h-full rounded-full flex items-center justify-end pr-4 transition-all duration-700 group-hover:bg-blue-400"
                              style={{ width: `${Math.max(percentage, 8)}%` }}
                            >
                              <span className="text-sm font-bold text-white">{day.views}</span>
                            </div>
                          </div>
                        </div>
                        <span className="text-xs text-ink-2 w-20 bg-black/5 px-3 py-1.5 rounded-full text-center font-medium">
              ...
              <div className="text-center py-16">
                ...
                <p className="text-ink-2">Aún no hay datos suficientes</p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="surface rounded-3xl p-6 shadow-sm">
              ...
              {interactions.length > 0 ? (
                <>
                  ...
                  <div className="relative w-40 h-40 mx-auto mb-6">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <div className="text-3xl font-extrabold text-ink">{totalInteractions}</div>
                  ...
                  <div className="space-y-3">
                    {interactions.map((interaction, index) => {
                      ...
                      return (
                        <div key={index} className="flex items-center gap-3 bg-black/[0.02] rounded-2xl p-3 border border-black/8">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-ink-2">{interaction.interaction_count} veces</p>
                          </div>
                          <div className="text-xl font-bold text-ink">{Math.round((interaction.interaction_count / totalInteractions) * 100)}%</div>
                        </div>
                      )
                    })}
                  </div>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-ink-2">Sin interacciones aún</p>
                </div>
              )}
            </div>
            ...
            <div className="bg-amber-50 rounded-3xl border border-amber-200 p-6 shadow-sm">
              <h3 className="text-lg font-bold text-ink mb-4 flex items-center gap-3">
              ...
              {analytics.last_viewed_at ? (
                ...
                <p className="text-sm text-ink-2">Sin visitas aún</p>
              )}
            </div>
