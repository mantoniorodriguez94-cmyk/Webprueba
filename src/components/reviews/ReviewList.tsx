...
export default function ReviewList({ reviews, loading = false }: ReviewListProps) {
  if (loading) {
    return (
      <div className="space-y-4">
        {[1, 2, 3].map((i) => (
          <div key={i} className="animate-pulse bg-transparent rounded-3xl p-6 border border-white/20">
  ...
  if (reviews.length === 0) {
    return (
      <div className="text-center py-16 bg-transparent backdrop-blur-sm rounded-3xl border border-white/20">
...
function ReviewCard({ review }: { review: Review }) {
  ...
  return (
    <div className="bg-transparent backdrop-blur-sm rounded-3xl p-6 border border-white/20 hover:border-blue-500/50 transition-all duration-300">
      <div className="flex items-start gap-4">
        {/* Avatar */}
